...
try {
  // pnpm can extract non-bin package files without executable bits. Restore
  // owner-only execution only after provenance and content verification, so a
  // missing or tampered payload is never made executable by this wrapper.
  fs.chmodSync(binaryPath, 0o500);
} catch (error) {
  fail(`failed to secure embedded binary permissions: ${error.message}`);
}
