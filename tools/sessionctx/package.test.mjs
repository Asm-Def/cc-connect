...
function createFixture(t) {
  ...
  fs.writeFileSync(path.join(dir, "bin", "cc-connect"), binary, { mode: 0o755 });
...
test("runner fails closed for missing binary or checksum", (t) => {
  ...
  fs.writeFileSync(path.join(dir, "bin", "cc-connect"), "x", { mode: 0o755 });
...
test("local tarball installs with pnpm offline and contains no downloader", (t) => {
  const dir = createFixture(t);
  const out = fs.mkdtempSync(path.join(os.tmpdir(), "cc-sessionctx-pack-"));
  const prefix = fs.mkdtempSync(path.join(os.tmpdir(), "cc-sessionctx-prefix-"));
  ...
  fs.writeFileSync(path.join(prefix, "package.json"), '{"name":"offline-prefix","private":true}\n');
  execFileSync("pnpm", ["add", "--offline", "--force", tarball], {
    cwd: prefix,
    stdio: "pipe",
    env: {
      ...process.env,
  ...
  const installed = path.join(prefix, "node_modules", "cc-connect");
